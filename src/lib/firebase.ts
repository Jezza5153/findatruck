...
console.log("--------------------------------------------------------------------------------");
console.log(`- NEXT_PUBLIC_FIREBASE_API_KEY: ${rawApiKey ? `SET (ends with ...${rawApiKey.slice(-5)})` : 'NOT SET or EMPTY'}`);
console.log(`- NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN: ${rawAuthDomain || 'NOT SET or EMPTY'}`);
console.log(`- NEXT_PUBLIC_FIREBASE_PROJECT_ID: ${rawProjectId || 'NOT SET or EMPTY'}`);
console.log(`- NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET: ${rawStorageBucket || 'NOT SET or EMPTY'}`);
console.log(`- NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID: ${rawMessagingSenderId || 'NOT SET or EMPTY'}`);
console.log(`- NEXT_PUBLIC_FIREBASE_APP_ID: ${rawAppId || 'NOT SET or EMPTY'}`);
console.log(`- NEXT_PUBLIC_FIREBASE_MEASUREMENT_ID: ${rawMeasurementId || 'NOT SET or EMPTY (Optional)'}`);
console.log("================================================================================");
...
if (getApps().length === 0) {
  try {
    ...
    console.log("Firebase app initialized successfully via initializeApp(). Project ID:", firebaseConfig.projectId);
  } catch (e: any) {
    console.error("CRITICAL: Firebase initializeApp FAILED. Error:", e.message);
    console.error("This usually means a value in your Firebase config (e.g., projectId, authDomain) is malformed, your API key is invalid for this project, or the Firebase services (Auth, Firestore) are not enabled or correctly set up in your Firebase project console for this project ID.");
    // @ts-ignore - app might not be initialized, leading to errors later
    app = null; 
  }
} else {
  app = getApps()[0];
  console.log("Firebase app already initialized (getApps().length > 0). Using existing app for Project ID:", app.options.projectId);
}
...
if (app && app.options && app.options.apiKey) {
  ...
  if (typeof window !== 'undefined' && firebaseConfig.measurementId && firebaseConfig.measurementId.startsWith('G-')) {
    ...
    console.warn(`Firebase Measurement ID ('${firebaseConfig.measurementId}') seems invalid (must start with 'G-'). Analytics not initialized.`);
  }
} else {
    console.error("Firebase app object is null or invalid after initialization attempt. Services (Auth, Firestore, Storage) will NOT be initialized correctly.");
