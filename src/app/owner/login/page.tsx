        ...
        <form onSubmit={handleSubmit(handleLogin)}>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="email-owner-login">Email address</Label>
              <Input id="email-owner-login" type="email" {...register("email")} placeholder="owner@example.com" />
              {errors.email && <p className="text-xs text-destructive mt-1">{errors.email.message}</p>}
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="password-owner-login">Password</Label>
                <Link href="#" passHref className="text-sm font-medium text-accent hover:underline">
                  Forgot your password?
                </Link>
              </div>
              <Input id="password-owner-login" type="password" {...register("password")} placeholder="••••••••" />
