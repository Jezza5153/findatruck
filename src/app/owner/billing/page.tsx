...
export default function OwnerBillingPage() {
  ...
  useEffect(() => {
    ...
    const fetchBillingData = async () => {
      ...
      try {
        ...
        // Placeholder data
        const fetchedSubscription: SubscriptionDetails | null =
          Math.random() > 0.3
            ? {
  ...
  const handleSubscribe = async () => {
    ...
    toast({
      ...
      description:
        'You are now on the Premium Monthly plan. Enjoy the features!',
    });
  };
  ...
  const handleCancelSubscription = async () => {
    ...
    if (
      !confirm(
        'Are you sure you want to cancel your subscription? Premium features will remain active until the end of the current billing period.'
      )
    )
      return;
    setIsSaving(true);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Card className="md:col-span-2 shadow-lg hover:shadow-xl transition-shadow">
          <CardHeader>
            ...
            <CardDescription>
              Unlock powerful tools to boost your truck's visibility and
              customer engagement for just $10/month.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <ul className="list-disc list-inside space-y-2 text-muted-foreground">
              <li>
                ...
                Featured Listing placement (top of map & search results).
              </li>
              <li>
                <CheckCircle className="inline mr-2 h-5 w-5 text-green-500" />
                Access to Loyalty Rewards program tools.
              </li>
              <li>
                <CheckCircle className="inline mr-2 h-5 w-5 text-green-500" />
                Detailed customer analytics and data export.
              </li>
              <li>
                <CheckCircle className="inline mr-2 h-5 w-5 text-green-500" />
            ...
            {subscription && subscription.status === 'active' ? (
              ...
              <>
                ...
                <p className="text-sm text-muted-foreground pt-2">
                  Your {subscription.planName} plan was cancelled. Access to
                  premium features will end after{' '}
                  {subscription.nextBillingDate || 'the current period'}.
                </p>
                <Button
                  size="lg"
              ...
              <>
                ...
                <p className="text-sm text-muted-foreground pt-2">
                  You are currently on the Free plan. Upgrade to Premium to
                  unlock all features.
                </p>
              </>
            )}
            <Button
              ...
              onClick={() =>
                toast({
                  ...
                  description: 'Secure payment management coming soon!',
                })
              }
              disabled={isSaving}
            >
      ...
      <Card className="mt-8 shadow-lg hover:shadow-xl transition-shadow">
          ...
          {paymentHistory.length > 0 ? (
            <ul className="space-y-3">
              {paymentHistory.map((invoice) => (
                <li
                  key={invoice.id}
                  className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 border rounded-md bg-muted/50"
                >
                  <div>
                    <p className="font-semibold">
                      Invoice {invoice.id} - {invoice.plan}
                    </p>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        toast({
                          ...
                          description: `Downloading ${invoice.id}.pdf (Feature Coming Soon)`,
                        })
                      }
            ...
            <p className="text-muted-foreground">
              No payment history found. Active subscriptions will show invoices
              here after billing.
            </p>
          )}
        </CardContent>
        <CardFooter>
          <p className="text-xs text-muted-foreground">
            For billing inquiries, please contact support via the Help page.
          </p>
        </CardFooter>
      </Card>
    </div>
