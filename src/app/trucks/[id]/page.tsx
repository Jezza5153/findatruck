...
export default function FoodTruckProfilePage() {
    ...
    if (truckId) {
      const fetchTruckDetailsAndMenu = async () => {
        ...
        try {
          ...
          const truckDocSnap: DocumentSnapshot<DocumentData> =
            await getDoc(truckDocRef);
          ...
          if (truckDocSnap.exists()) {
            ...
            const fetchedTruck: FoodTruck = {
              ...
              imageUrl:
                data.imageUrl ||
                `https://placehold.co/800x400.png?text=${encodeURIComponent(data.name || 'Food Truck')}`,
              ownerUid: data.ownerUid || '',
              lat: typeof data.lat === 'number' ? data.lat : undefined,
              lng: typeof data.lng === 'number' ? data.lng : undefined,
              address: data.address || 'Location details not available.',
              operatingHoursSummary:
                data.operatingHoursSummary || 'Hours not specified.',
              isOpen:
                data.isOpen === undefined ? undefined : Boolean(data.isOpen),
              rating: typeof data.rating === 'number' ? data.rating : 0, // Default to 0 if undefined
              numberOfRatings:
                typeof data.numberOfRatings === 'number'
            ...
            const fetchedMenuItems: MenuItemType[] = menuItemsSnap.docs.map(
              (docSnap) =>
                ({
                  id: docSnap.id,
                  ...docSnap.data(),
          ...
          toast({
            title: 'Error',
            description: err.message || 'Could not load truck.',
            variant: 'destructive',
          });
        } finally {
          setIsLoading(false);
      ...
      toast({
        ...
        action: (
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push(`/login?redirect=/trucks/${truckId}`)}
          >
            Login
          </Button>
        ),
    ...
    try {
      if (isFavorite) {
        ...
        toast({
          ...
          description: `${truck.name} is no longer in your favorites.`,
        });
      } else {
        await updateDoc(userDocRef, { favoriteTrucks: arrayUnion(truck.id) });
        toast({
          title: 'Added to Favorites!',
          description: `${truck.name} is now in your favorites.`,
        });
      }
  ...
  const handleNotifyNearby = () => {
    if (!currentUser) {
      toast({
        title: 'Login Required',
        description: 'Please log in to set up notifications.',
        variant: 'default',
        action: (
          <Button
            variant="outline"
    ...
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <Alert className="max-w-md mx-auto">
          ...
          <AlertDescription>
            The food truck you are looking for could not be found.
          </AlertDescription>
        </Alert>
      </div>
    );
      ...
      <Card className="overflow-hidden shadow-xl">
        <div className="relative h-64 md:h-96">
          <Image
            src={
              truck.imageUrl ||
              `https://placehold.co/800x400.png?text=${encodeURIComponent(truck.name)}`
            }
            alt={truck.name}
            layout="fill"
            objectFit="cover"
            ...
            priority
            onError={(e) =>
              (e.currentTarget.src = `https://placehold.co/800x400.png?text=${encodeURIComponent(truck.name)}`)
            }
          />
          ...
          <div className="absolute bottom-0 left-0 p-6 md:p-8">
            ...
            <Badge
              variant="secondary"
              className="text-lg bg-white/25 text-white backdrop-blur-sm border-white/30 shadow-md"
            >
              {truck.cuisine}
            </Badge>
          </div>
          ...
          >
            <Star
              className={`h-6 w-6 ${isFavorite ? 'fill-yellow-400 text-yellow-400' : 'text-white'}`}
            />
          </Button>
        </div>
        ...
        <CardContent className="pt-6">
          <div className="grid md:grid-cols-3 gap-x-8 gap-y-6 mb-8">
              ...
              <div className="space-y-2 text-muted-foreground">
                {truck.rating && truck.rating > 0 && (
                  <div className="flex items-center">
                    ...
                    {truck.rating.toFixed(1)} stars{' '}
                    {truck.numberOfRatings
                      ? `(${truck.numberOfRatings} ratings)`
                      : ''}
                  </div>
                )}
                <div className="flex items-center">
                ...
                {truck.isOpen !== undefined && (
                  <Badge
                    variant={truck.isOpen ? 'default' : 'destructive'}
                    className={`mt-2 ${truck.isOpen ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}
                  >
            ...
            <div className="md:col-span-1 space-y-3">
              <Button
                ...
                disabled={truck.isOpen === false || menuItems.length === 0}
              >
                <ShoppingCart className="mr-2 h-5 w-5" /> Order Now (Coming
                Soon)
              </Button>
              <Button
                size="lg"
          ...
          {menuItems && menuItems.length > 0 ? (
            <Tabs
              defaultValue={menuCategories[0] || 'all-items'}
              className="w-full"
            >
            ...
            <Card className="my-6">
              ...
              <CardContent className="pt-0 text-center text-muted-foreground">
                {' '}
                This truck's menu is not available at the moment. Please check
                back later!{' '}
              </CardContent>
            </Card>
          )}
                    ...
                    <CardContent className="pt-6">
                      {testimonial.avatarUrl && (
                        <Image
                          ...
                          width={40}
                          height={40}
                          className="rounded-full mb-2 float-left mr-3"
...
const ScrollArea = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<
    typeof import('@radix-ui/react-scroll-area').Root
  >
