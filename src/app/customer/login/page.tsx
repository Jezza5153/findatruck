      ...
      <Card className="w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={handleSubmit(handleLogin)}>
          <CardContent className="space-y-4">
            <div>
              <Label htmlFor="email-customer-login">Email address</Label>
              <Input id="email-customer-login" type="email" {...register("email")} placeholder="you@example.com" />
              {errors.email && <p className="text-xs text-destructive mt-1">{errors.email.message}</p>}
            </div>
            <div>
              <div className="flex items-center justify-between">
                <Label htmlFor="password-customer-login">Password</Label>
                <Link href="#" passHref className="text-sm font-medium text-primary hover:underline">
                  Forgot your password?
                </Link>
              </div>
              <Input id="password-customer-login" type="password" {...register("password")} placeholder="••••••••" />
